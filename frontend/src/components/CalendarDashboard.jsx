...
const CalendarDashboard = () => {
  ...
  const fetchCalendarEntries = useCallback(async () => {
    ...
    try {
      ...
      console.log("Loaded calendar entries for", selectedPlatform, ":", data?.length)
      setEntries(data || [])
                  ...
                  >
                    ...
                    {hasEntries && (
                      <div className="space-y-2 overflow-y-auto h-full pt-6">
                        ...
                        {dayEntries.length > 3 && (
                          <div className={`text-sm px-2 py-1 rounded text-center font-medium cursor-pointer hover:opacity-80 transition-opacity ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                            }`}
