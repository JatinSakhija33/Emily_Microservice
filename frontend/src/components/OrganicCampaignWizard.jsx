            ...
            <div>
                ...
                <div className="flex gap-2">
                    {[14, 30, 60, 90].map(d => (
                        <button
                            key={d}
                            onClick={() => handleInputChange('duration_days', d)}
                            className={`flex-1 py-2 rounded-lg border text-sm font-medium transition-colors ${formData.duration_days === d
    ...
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
            ...
            <p className="text-sm text-blue-700 dark:text-blue-200">
                We will generate a <strong>{formData.duration_days}-day</strong> campaign
                targeting <strong>{formData.goal ? GOALS.find(g => g.value === formData.goal).label : 'your goal'}</strong>.
                <br />
                Approximately <strong>{Math.floor(formData.duration_days / 7 * (formData.frequency === 'daily' ? 7 : 3.5)) * formData.platforms.length}</strong> posts
