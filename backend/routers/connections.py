...
from routers.whatsapp_connections import router as whatsapp_router
...
async def debug_whatsapp_config():
    ...
    return {
        "whatsapp_app_id_configured": bool(whatsapp_app_id),
        "whatsapp_app_secret_configured": bool(whatsapp_app_secret),
        "whatsapp_redirect_uri": whatsapp_redirect_uri,
        "message": "Check if all values are true/non-null"
    }
...
async def initiate_whatsapp_connection(current_user: User = Depends(get_current_user)):
    ...
    """
    try:
        logger.info(f"Initiating WhatsApp connection for user {current_user.id}")

        WHATSAPP_APP_ID = os.getenv("WHATSAPP_APP_ID")
        WHATSAPP_API_VERSION = os.getenv("WHATSAPP_API_VERSION", "v21.0")
        WHATSAPP_AUTH_URL = f"https://www.facebook.com/{WHATSAPP_API_VERSION}/dialog/oauth"

        logger.info(f"WHATSAPP_APP_ID configured: {bool(WHATSAPP_APP_ID)}")

        if not WHATSAPP_APP_ID:
            logger.error("WHATSAPP_APP_ID environment variable not set")
            raise HTTPException(
...
def generate_oauth_url(platform: str, state: str) -> str:
    ...
    if platform == 'facebook':
        ...
        # Using simplified scopes that don't require advanced access approval
